...
#define IM2COL(bda) \
    ...
    ggml_vk_create_pipeline(device, device->pipeline_ssm_scan_m1_f32_d16, "ssm_scan_m1_16_f32", ssm_scan_m1_f32_len, ssm_scan_m1_f32_data, "main", 8, sizeof(vk_op_ssm_scan_push_constants), {128, 1, 1}, {16, 128}, 1);
...
static vk_pipeline ggml_vk_op_get_pipeline(ggml_backend_vk_context * ctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * dst, ggml_op op) {
    ...
    case GGML_OP_SSM_SCAN:
        if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
            ...
            if (dst->src[3] && dst->src[3]->nb[1] != sizeof(float)) {
                // Mamba-1: A is {d_state, n_head}
                return d_state == 16 ? ctx->device->pipeline_ssm_scan_m1_f32_d16 : nullptr;
            }
            if (d_state == 64) {
...
void ggml_vk_ssm_scan(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_tensor * dst) {
    ...
    if (is_mamba2) {
        ...
        elements = { num_workgroups_x, n_seq, 1 };
    } else {
        // Mamba-1: one invocation per channel (head_dim == 1)
        ...
        case GGML_OP_SSM_SCAN:
            {
                ...
                if (!is_mamba2) {
                    // Mamba-1: d_state 16, one channel per head, no rollback snapshots
                    return d_state == 16 && head_dim == 1 && ggml_get_op_params_i32(op, 0) == 1 &&
                           op->src[4]->nb[1] == d_state * sizeof(float) && op->src[5]->nb[1] == d_state * sizeof(float);
                }
