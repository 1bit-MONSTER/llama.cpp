...
static bool is_swiglu_params(const OpParams & params) {
...
static void add_common_routed_down_compile_parameters(Dispatch & dispatch, int64_t token_count, ggml_type weight_type) {
    dispatch.kernel.compile_parameters.emplace("ggml.mul_mat_id_f16_f16.input_size",
                                               to_config_value(kRoutedFfnExpertHiddenSize));
    dispatch.kernel.compile_parameters.emplace("ggml.mul_mat_id_f16_f16.route_count",
                                               to_config_value(kRoutedFfnRouteCount));
    dispatch.kernel.compile_parameters.emplace("ggml.mul_mat_id_f16_f16.expert_count",
                                               to_config_value(kRoutedFfnExpertCount));
    dispatch.kernel.compile_parameters.emplace("ggml.mul_mat_id_f16_f16.output_size",
                                               to_config_value(kRoutedFfnInputSize));
    dispatch.kernel.compile_parameters.emplace("ggml.mul_mat_id_f16_f16.weight_format",
                                               weight_type == GGML_TYPE_Q4_K ? "4" : "6");
...
static RoutedDownMatch match_routed_ffn_down_grouped(const DispatchMatchContext & context) {
    ...
    match.kernel            = kCommonMulMatIdF16F16WmmaKernel;
...
static RoutedGateUpMatch match_routed_ffn_gate_up_swiglu(const DispatchMatchContext & context) {
    ...
    if (!is_swiglu_params(glu_node->params)) {
        return {};
    }
...
static bool build_routed_ffn_down_grouped_dispatch(const DispatchMatchContext & context,
    ...
    if (!match.matched() || match.weight->type != expected_weight_type) {
        return false;
    }
    ...
    dispatch.kernel = make_kernel_specialization(match.kernel);
    dispatch.kernel.integer_parameters.emplace("token_count", match.token_count);
    add_common_routed_down_compile_parameters(dispatch, match.token_count, match.weight->type);
    dispatch.bindings.push_back({ match.input_alternate->alternate_value, 0, match.input_alternate->byte_count });
    dispatch.bindings.push_back(
        { match.routing_bundle->expert_table, 0, match.routing_bundle->expert_table_byte_count });
    dispatch.bindings.push_back({ match.weight->id, 0, match.weight->byte_count });
...
static bool match_routed_ffn_down_q4k_f16_wmma_grouped_dispatch(const DispatchMatchContext & context,
                                                                DispatchMatch &              dispatch_match) {
    return build_routed_ffn_down_grouped_dispatch(context, dispatch_match, GGML_TYPE_Q4_K);
}

static bool match_routed_ffn_down_q6k_f16_wmma_grouped_dispatch(const DispatchMatchContext & context,
                                                                DispatchMatch &              dispatch_match) {
    return build_routed_ffn_down_grouped_dispatch(context, dispatch_match, GGML_TYPE_Q6_K);
}
