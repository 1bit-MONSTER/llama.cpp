...
// layer's expert FFN runs on the CPU out of pinned RAM slots that an O_DIRECT reader fills on
// demand, so a model's experts need not fit in RAM. Enabled by environment:
...
// The expert FFN of layer il with SwiGLU, from the streamed slots: returns the per-expert down
